 ...
 */
export const configureMiddleware = (app: Express, frontendUrl: string) => {
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
      ...
      console.log(`[${logLevel.toUpperCase()}] ${req.method} ${req.path} - ${res.statusCode} (${duration}ms) [ID: ${(req as any).id}]`);
    });
