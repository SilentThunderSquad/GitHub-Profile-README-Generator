 ...
 */
export const getReadmeById = async (readmeId: string, userId?: string): Promise<README> => {
  const { data: readme, error } = await supabase
    .from('readmes')
    .select(`
      *,
 ...
 * Update README
 */
export const updateReadme = async (readmeId: string, userId: string, updates: Partial<README>): Promise<README> => {
  // Verify ownership
 ...
 * Delete README
 */
export const deleteReadme = async (readmeId: string, userId: string): Promise<void> => {
 ...
 * Get user's READMEs
 */
export const getUserReadmes = async (userId: string, limit = 20, offset = 0) => {
 ...
 */
export const getUserPublicReadmes = async (userId: string, limit = 20, offset = 0) => {
  const { data, error, count } = await supabase
    .from('readmes')
    .select('*', { count: 'exact' })
    .eq('user_id', userId)
 ...
 * Publish README
 */
export const publishReadme = async (readmeId: string, userId: string): Promise<README> => {
  const { data: readme } = await supabase.from('readmes').select('user_id').eq('id', readmeId).single();
 ...
 * Autosave README
 */
export const autosaveReadme = async (readmeId: string, userId: string, blocks: Block[]): Promise<README> => {
  const { data: readme } = await supabase.from('readmes').select('user_id').eq('id', readmeId).single();
 ...
 */
export const changeReadmeVisibility = async (
  readmeId: string,
  userId: string,
 ...
 * Add tags to README
 */
export const addReadmeTags = async (readmeId: string, userId: string, tagSlugs: string[]): Promise<void> => {
  // Verify ownership
  const { data: readme } = await supabase.from('readmes').select('user_id').eq('id', readmeId).single();
 ...
 */
export const getPublicReadmeWithAuthor = async (readmeId: string): Promise<any> => {
