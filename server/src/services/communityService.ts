 ...
 */
export const getTrendingReadmes = async (limit = 20, offset = 0, filters?: Partial<SearchFilters>) => {
  let query = supabase
    .from('readmes')
    .select(`
      *,
  ...
  const scored = (data || []).map((readme) => ({
    ...
    trendingScore: calculateTrendingScore(
      readme.clone_count || 0,
      readme.like_count || 0,
      getDaysSince(readme.created_at)
    ),
  }));
 ...
 */
export const searchReadmes = async (filters: SearchFilters, limit = 20, offset = 0) => {
  let query = supabase
    .from('readmes')
    .select(`
      *,
 ...
 */
export const getTemplatesByCategory = async (categorySlug: string, limit = 20, offset = 0) => {
 ...
 */
export const incrementViewCount = async (readmeId: string) => {
